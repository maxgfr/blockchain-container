...
async function positionReading(positionReading) {
    ...
    for (let n = 0; n < results.length; n++) {
        ...
        if (distance_metres <= geofenc_site) {
            ...
            if (container.site) {
                let container_site = container.site;
                if (container_site[container_site.length - 1] != site)  { // site changing
...
async function setupDemo(setupDemo) {
    ...
    let site_container = factory.newRelationship(NS, 'Site', 'site@email.com');
    container.site = [site_container];
