...
async function positionReading(positionReading) {
    ...
    for (let n = 0; n < results.length; n++) {
        ...
        if (geofenc_site < distance_metres) {
            ...
            container.contract.responsible = site.responsible;
        } else {
            // set the status of the container
            container.status = 'OFF_SITE';
        }

        console.log('Responsible: ' + container.contract.responsible.$identifier);
        console.log('Owner: ' + container.contract.owner.$identifier);

        // Emit an event for the contract.
        let notif = getFactory().newEvent('org.magma.max', 'hasChanged');
        notif.asset = container;
    ...
    await responsibleRegistry.update(container.contract.responsible);
...
async function setupDemo(setupDemo) {
    ...
    const container = factory.newResource(NS, 'Container', 'SHIP_001');
    container.status = 'IN_SITE';
    container.contract = factory.newRelationship(NS, 'Contract', 'CON_001');
