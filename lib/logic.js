...
async function positionReading(positionReading) {
    ...
    for (let n = 0; n < results.length; n++) {
        ...
        if (geofenc_site < distance_metres) {
            ...
            const ar = await getAssetRegistry('org.magma.max.Responsible');
            await ar.update(site.responsible);
            //container.responsible = site.responsible;
        } else {
            // set the status of the container
    ...
    await responsibleRegistry.update(container.responsible);
